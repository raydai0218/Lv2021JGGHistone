...
## 1.1 安装CRAN来源常用包
  ...
  package_list = c("vegan", "reshape2", "ggplot2", "devtools", "bindrcpp", "ggthemes", "agricolae", "dplyr", "RColorBrewer",
                   "scales", "vegan", "pheatmap","BiocManager","psych", "igraph", "VennDiagram", "factoextra")
...
## 2.2 安装bioconductor常用包
  ...
  package_list = c("digest", "ggrepel","WGCNA")
  for(p in package_list){
    if(!suppressWarnings(suppressMessages(require(p, character.only = TRUE, quietly = TRUE, warn.conflicts = FALSE)))){
